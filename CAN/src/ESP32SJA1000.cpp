...
int ESP32SJA1000Class::endPacket()
{
    ...
    Serial.println("antes tx");
    // wait for TX complete
    int time_old = millis();
    while ((readRegister(REG_SR) & 0x08) != 0x08)
    {

        //yield(); //pesquisar funcionamento
        
        if (millis() - time_old > 500)
        {
            if (readRegister(REG_ECC)){
                //¨¨VERIFICAR SE É PRECISO MODIFICAR REGISTRADOR
                modifyRegister(REG_CMR, 0x1f, 0x02); // error, abort
            }
            
            //wait 500 mseg
            return readRegister(REG_ECC);
        }
    }
    Serial.println("depois tx");
